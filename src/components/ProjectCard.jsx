...
const ProjectCard = ({ limit }) => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await axios.get('https://skillcrafted-backend.vercel.app/projects/projectList/');
  ...
  const fetchFilteredProjects = async () => {
    ...
    try {
      const res = await axios.get(`https://skillcrafted-backend.vercel.app/projects/projectList/${query}`);
              ...
              return (
                <div key={project.id}> 
                  <section className='flex flex-wrap px-3 justify-start md:gap-5'>
                    <div className="w-[280px] shadow-lg rounded-md p-5">
